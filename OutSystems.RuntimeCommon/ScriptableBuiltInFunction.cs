...
namespace OutSystems.RuntimeCommon {
    ...
    public static class ScriptableBuiltInFunction {
        ...
        public static decimal Trunc(decimal n) {
            try {
                if (n >= 0M)
                    return Convert.ToDecimal(Math.Floor(Convert.ToDouble(n)));
                else
                    return Convert.ToDecimal(Math.Ceiling(Convert.ToDouble(n)));
            } catch {
                return Decimal.Zero;
            }
        }
        ...
        public static int DiffHours(DateTime dt, DateTime dt2) {
            try {
                DateTime d = dt.AddMinutes(-dt.Minute).AddSeconds(-dt.Second);
                DateTime d2 = dt2.AddMinutes(-dt2.Minute).AddSeconds(-dt2.Second);
        ...
        public static int DiffMinutes(DateTime dt, DateTime dt2) {
            try {
                DateTime d = dt.AddSeconds(-dt.Second);
                DateTime d2 = dt2.AddSeconds(-dt2.Second);
        ...
        public static int DiffSeconds(DateTime dt, DateTime dt2) {
            try {
                return (int) dt2.Subtract(dt).TotalSeconds;
            } catch {
                return 0;
            }
        }
        ...
        public static string DecimalToText(decimal d) {
            try {
                // This may look odd, but 'G' by itself will not trim zeroes, using "0.####" will normally work, but still won't collapse some nums
                //  and doing the trim in one go as TrimEnd( new array[] { '0', '.'} ) will wipe "0.000"...
                return d.ToString("N", FormatInfoLogic.Number).TrimEnd('0').TrimEnd(FormatInfoLogic.DecimalSeparator);
            } catch {
                return string.Empty;
            }
        }

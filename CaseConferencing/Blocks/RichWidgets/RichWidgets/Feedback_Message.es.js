    ...
    outsystems.internal.$(function($){
        if(!RichWidgets_Feedback_Message_UnloadingState)    {
            if (!RichWidgets_Feedback_Message_errorTrapped)
                OsNotifyWidget(RichWidgets_Feedback_Message_notifyWidget, '3) Se produjo una excepción en el script de cliente <br\> Error:' + (exception.message == ''? exception : exception.message) );

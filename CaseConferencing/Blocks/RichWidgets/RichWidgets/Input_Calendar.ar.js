...
Calendar.is_khtml = false;

// BEGIN: UTILITY FUNCTIONS; beware that these might be moved into a separate
//        library, at some point.

Calendar.getAbsolutePos = function(el) {
 if (typeof(outsystems.internal.$) != 'undefined') {
  Calendar.getAbsolutePos = function(el) {
   var o = outsystems.internal.$(el).offset();
   return { x: o.left, y: o.top };
  };
  return Calendar.getAbsolutePos(el);
 } else {
  Calendar.getAbsolutePos = function(el) {
   var SL = 0, ST = 0;
...
// FIXME: Consider replacing the following 2 functions 
...
Calendar.prototype.create = function (_par) {
 ...
 this._nav_pm = hh("&#x2039;", 1, -1);
 this._nav_pm.className = "button notclose";
 this._nav_pm.ttip = Calendar._TT["PREV_MONTH"];

 this._nav_now = hh(Calendar._TT["TODAY"], this.weekNumbers ? 4 : 3, 0);
 this._nav_now.className = "button notclose"; 
 this._nav_now.ttip = Calendar._TT["GO_TODAY"];

 this._nav_nm = hh("&#x203a;", 1, 1);
 this._nav_nm.className = "button notclose"; 
...
Calendar._keyEvent = function(ev) {
  ...
  if (ev.ctrlKey) {
    switch (K) {
      case 37: // KEY left
        act && Calendar.cellClick(cal._nav_pm); cal.change = act;
        break;
      case 38: // KEY up
        act && Calendar.cellClick(cal._nav_py); cal.change = act;
        break;
      case 39: // KEY right
        act && Calendar.cellClick(cal._nav_nm); cal.change = act;
        break;
      case 40: // KEY down
        act && Calendar.cellClick(cal._nav_ny); cal.change = act;
        break;
      default:
        return false;
    }
    ...
    case 13: // KEY enter
      if (act) {
        Calendar.cellClick(cal.currentDateEl, ev); cal.change = act;
      }
      break;
    default:
      return false;
...
Calendar.setup = function (params) {
 if (typeof (this.useNativePicker) === "undefined") {
  ...
  this.useNativePicker = /mobile/i.test(nua);
  this.isAndroidDefaultBrowser = ((nua.indexOf('Mozilla/5.0') > -1 && nua.indexOf('Android ') > -1 && nua.indexOf('AppleWebKit') > -1) && !(nua.indexOf('Chrome') > -1));
 }
 if (this.isAndroidDefaultBrowser && params["showsTime"]) {//#679168 Android default browser does not support DateTime Pickers
 ...
 if (this.useNativePicker && this.supportedNativePickers[pickerTypeIfNative]) {
  ...
  var inputVal = document.getElementById(params["inputField"]).getAttribute("value");
  if (inputVal != null) {
      var parsedDate = getDateFromFormat(inputVal, _DATE_TIME_FORMAT);
  ...
  return null;
 } else if (!this.isAndroidDefaultBrowser && !!('ontouchstart' in window) && typeof outsystems.internal.$ !== "undefined") { //mobile phones 
  var input = outsystems.internal.$('#' + params["inputField"]);
  input.attr('readonly', 'true');
  input.css('background', 'none');
  input.click(function() {
   var top = outsystems.internal.$(this).offset().top;
...
Calendar._TT["PREV_YEAR"] = "العام السابق (عقد لمدة القائمة)";
Calendar._TT["PREV_MONTH"] = "الشهر السابق (عقد لمدة القائمة)";
Calendar._TT["GO_TODAY"] = "اليوم";
Calendar._TT["NEXT_MONTH"] = "الشهر القادم (عقد لمدة القائمة)";
Calendar._TT["NEXT_YEAR"] = "العام المقبل (عقد لمدة القائمة)";
Calendar._TT["SEL_DATE"] = "اختر التاريخ";
Calendar._TT["DRAG_TO_MOVE"] = "اسحب لتحريك";
Calendar._TT["PART_TODAY"] = " (today)";

// the following is to inform that "%s" is to be the first day of week
// %s will be replaced with the day name.
Calendar._TT["DAY_FIRST"] = "العرض بالنسبة  %  أولا";
...
Calendar._TT["CLOSE"] = "أغلق";
Calendar._TT["TODAY"] = "اليوم";
Calendar._TT["TIME_PART"] = " اضغط (shift)/ اسحب للتغيير";
// date formats
...
window.RichWidgets_Calendar = Calendar;

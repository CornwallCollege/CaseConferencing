...
namespace OutSystems.HubEdition.RuntimePlatform.Processes {
    ...
    public abstract class BPMRuntime {
            ...
            using (Transaction trans = DatabaseAccess.ForSystemDatabase.GetRequestTransaction()) {
                ...
                using (IDataReader reader = DBRuntimePlatform.Instance.GetActivity(trans, tenantId, activityId, true)) {
                    if (reader.Read()) {
                        ...
                        runnable = (isRunningSince == BuiltInFunction.NullDate())
                                   && (!forInitialStatus.HasValue || forInitialStatus.Value == currentStatus)
                                   && (previousNextRun == BuiltInFunction.NullDate() || Math.Abs((previousNextRun - nextRun).TotalMilliseconds) < 1);
        ...
        public static void GetActivityDataForWS(int activityId, out int processId, out ActivityKind activityKind, out string activityName, out string SSKey, out string url, out int tenantId, out string EspaceName, out string processName) {
            using (Transaction trans = DatabaseAccess.ForSystemDatabase.GetRequestTransaction()) {
        ...
        public static void GetProcessDataForWS(int processId, out string SSKey, out string url, out int tenantId) {
            string EspaceName;
        ...
        private static string GetUrlForWS(string espaceName) {
            return "http://" + Settings.Get(Settings.Configs.InternalAddress) + "/" + espaceName + "/";
        }
        ...
        public static void FlushPendingActivityEvent(int activityId, out int processedEvents){
            ...
            using (Transaction trans = DatabaseAccess.ForSystemDatabase.GetCommitableTransaction()) {
                using (IDataReader reader = DBRuntimePlatform.Instance.DequeueEvent(trans, 1, "", activityId)) {
                    if (reader.Read()) {

...
namespace OutSystems.HubEdition.RuntimePlatform {
    ...
    public partial class DBRuntimePlatform {
        ...
        public bool HasParentProcess(Transaction tran, int processId, out int parentProcessId, out int parentActivityId, out string espaceKey, out string key) {
            Command cmd = tran.CreateCommand(
                "SELECT ESPACE.SS_KEY AS ESPACEKEY, PROCESS.PARENT_PROCESS_ID, PROCESS.PARENT_ACTIVITY_ID, ACTIVITYDEF.SS_KEY " +
                "FROM OSSYS_BPM_ACTIVITY ACTIVITY " +
                "INNER JOIN OSSYS_BPM_ACTIVITY_DEFINITION ACTIVITYDEF ON ACTIVITYDEF.ID = ACTIVITY.ACTIVITY_DEF_ID " +
                "INNER JOIN OSSYS_BPM_PROCESS PROCESS ON PROCESS.PARENT_ACTIVITY_ID = ACTIVITY.ID " +
                "INNER JOIN OSSYS_BPM_PROCESS_DEFINITION PROCESSDEF ON PROCESSDEF.ID = PROCESS.PROCESS_DEF_ID " +
                "INNER JOIN OSSYS_ESPACE ESPACE ON ESPACE.ID IN (" +
                "   SELECT E.ID" +
                "   FROM OSSYS_BPM_ACTIVITY A" +
                "   INNER JOIN OSSYS_BPM_PROCESS P ON P.ID = A.PROCESS_ID" +
        ...
        public IDataReader GetActivityDataForWS(Transaction tran, int activityId) {
            Command cmd = tran.CreateCommand(
                "SELECT E.NAME AS ESPACE_NAME, AD.SS_KEY, ACT.TENANT_ID AS TENANT_ID, ACT.PROCESS_ID, AK.ID AS ACTIVITYKIND, AD.NAME AS ACTIVITYNAME, PD.NAME AS PROCESSNAME " +
        ...
        public IDataReader GetProcessDataForWS(Transaction tran, int processId) {
            Command cmd = tran.CreateCommand(
                "SELECT E.NAME AS ESPACE_NAME, PD.SS_KEY, P.TENANT_ID AS TENANT_ID " +
                "FROM OSSYS_BPM_PROCESS P " +
                "INNER JOIN OSSYS_BPM_PROCESS_DEFINITION PD ON PD.ID = P.PROCESS_DEF_ID " +
                "INNER JOIN OSSYS_ESPACE E ON PD.ESPACE_ID = E.ID AND E.IS_ACTIVE = 1 " +
        ...
        public void UpdateHumanActivityParams(Transaction tran, int activityId, int? userId, int? groupId, DateTime? dueDate, DateTime? activationInstant, string inboxDetail) {
            ...
            if (inboxDetail != null) {
                cmd.CreateParameter("@INBOXDETAIL", DbType.String, RuntimePlatformUtils.Trunc(inboxDetail, 100));
            }
            cmd.CreateParameter("@ACTIVITYID", DbType.Int32, activityId);
        ...
        public int CreateActivity(Transaction tran, int tenantId, int activityDefId, int processId, string name, int userId, int precedentActivityId, string precedentOutcome, DateTime? dueDate, string inboxDetail) {
            ...
            string nextRunSnippet = GetNextRunSql(dml);
        ...
        public void UpdateActivity(Transaction tran, int? tenantId, int activityId, int? activityDefId, int? processId, string name, int? userId,
            ...
            if (hasError.HasValue && hasError.Value) {
                sqlCmd.CreateParameter("@BACKOFF", DbType.Decimal, Settings.GetDecimal(Settings.Configs.Scheduler_ProcessBackoffConstant));
                sqlCmd.CreateParameter("@RETRYBASESEC", DbType.Int32, Settings.GetInt(Settings.Configs.Scheduler_ProcessBackoff_RetrybaseSec));
            }
        ...
        [MethodImpl(MethodImplOptions.NoInlining)]
        [TestSyntaxValues(Values = new[] { TestSyntaxValues.AUTO, "1", TestSyntaxValues.AUTO, "1", "1", TestSyntaxValues.AUTO, TestSyntaxValues.AUTO, TestSyntaxValues.AUTO, "Active" })]
        public int CreateProcess(Transaction tran, int tenantId, DateTime created, int createdBy, int processDefId, int parentProcessId, int parentActivityId, int topProcessId, ProcessStatus status) {
            string sql =
                ...
                "VALUES (@TENANT_ID, @SSCREATED, @SSCREATED_BY, @SSPROCESS_DEF_ID, @SSPARENT_PROCESS_ID, " +
                "@SSPARENT_ACTIVITY_ID, @SSTOP_PROCESS_ID, @SSSTATUS_ID, @SSLAST_MODIFIED, @SSLAST_MODIFIED_BY)";

            Command sqlCmd = tran.CreateCommand(GetNoCountSQL(sql, true));
            sqlCmd.CreateParameter("@TENANT_ID", DbType.Int32, tenantId);
            sqlCmd.CreateParameter("@SSCREATED", DbType.DateTime, created);
            if (createdBy == 0) {
        ...
        public void DeleteEvent(Transaction trans, int id) {
            Command cmd = trans.CreateCommand("DELETE FROM OSSYS_BPM_EVENT_QUEUE WHERE ID=@ID");
            cmd.CreateParameter("@ID", DbType.Int32, id);
            cmd.ExecuteNonQuery();
        }
        ...
        public void RequeueEvent(Transaction trans, int id, bool hasError) {
            ...
            string sql = "UPDATE OSSYS_BPM_EVENT_QUEUE SET DEQUEUE_TIME=NULL " +
                         (hasError
            ...
            cmd.CreateParameter("@ID", DbType.Int32, id);
            if (hasError) {
                cmd.CreateParameter("@BACKOFF", DbType.Decimal, Settings.GetDecimal(Settings.Configs.Scheduler_ProcessBackoffConstant));
                cmd.CreateParameter("@RETRYBASESEC", DbType.Int32, Settings.GetInt(Settings.Configs.Scheduler_ProcessBackoff_RetrybaseSec));
            }
            cmd.ExecuteNonQuery();
        }
        
        protected virtual string GetDequeueEventSQL() {
            return "DEQUEUE_OSSYS_EVENT";
        }
        ...
        [MethodImpl(MethodImplOptions.NoInlining)]
        public IDataReader DequeueEvent(Transaction trans, int batch, string ip, int activityId) {
            Command cmd = trans.CreateCommand(GetDequeueEventSQL());
            
            cmd.CreateParameter("@BATCHSIZE", DbType.Int32, batch);
            cmd.CreateParameter("@IP", DbType.String, ip);

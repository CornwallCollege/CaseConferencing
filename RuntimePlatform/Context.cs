...
namespace OutSystems.HubEdition.RuntimePlatform {
    ...
    public class SessionDefaults {
        ...
        static public event GetSessionDefaultDelegate SessionDefaultFunctions;
    ...
    public class SessionInfo {
        ...
        // The user variables
        private SessionHashtable _variables;
        ...
        public SessionInfo() {
            _variables = new SessionHashtable();
        ...
        private HashSet<int> _roleCacheByUserId = null;
        private HashSet<int> RoleCache {
            get {
                if(_roleCacheByUserId == null) {
                    if (UserId == 0) {
                        _roleCacheByUserId = new HashSet<int>();
                    } else {
        ...
        public bool CheckRole(int roleId) {
            return RoleCache.Contains(roleId);           
        }

        public void AddRole(int roleId) {
            RoleCache.Add(roleId);
        }
        ...
        private HashSet<int> ReadAllUserRoles() {
            ...
            using (Transaction tran = DatabaseAccess.ForSystemDatabase.GetReadOnlyTransaction()) {
                using (var reader = DBRuntimePlatform.Instance.GetAllUserRoles(tran, UserId, TenantId)) {
                    HashSet<int> allUserRoles = new HashSet<int>();
                    while (reader.Read()) {
                        allUserRoles.Add(Convert.ToInt32(reader[0]));
                    }
                    return allUserRoles;
                }
            }
        ...
        private void InnerClearAndSet(bool onlyChangeSessionId, bool canKeepTenant, int? tenantId, int? userId, string userName) {
            ...
            if (onlyChangeSessionId) {
                TryChangeSessionId(/*sessionWasCleared*/false);
                ClearSessionStartedEspaces();
            } else {
                InnerClearSession();
                // Get a update instance and make changes there
                instance = AppInfo.GetAppInfo().OsContext.Session;
            }
            ...
            if (userId != null) {
                ...
                instance._variables[USER_ID_GUID] = 
                    MD5GuidHelper.ToGuidWithSaltAndUTF16(userId.Value.ToString(CultureInfo.InvariantCulture), SharedKeys.PrivateSalt()).ToString();
            }
        ...
        private void TryChangeSessionId(bool sessionWasCleared) {
            var context = AppInfo.GetAppInfo().OsContext;
            if (sessionWasCleared) {
                context.OnSessionCleared();
            }

            if (!MayChangeSessionId) {
                return;
            }

            string oldSessionId = this.SessionID;

            string newSessionId = ChangeSessionCookie();

            context.OnSessionIDChanged(newSessionId);

            if (!sessionWasCleared) {
                SqlSessionStateStore.ChangeSessionId(oldSessionId, newSessionId);
            }
        }

        private string ChangeSessionCookie() {
            SessionIDManager manager = new SessionIDManager();
            string newSessionId = manager.CreateSessionID(HttpContext.Current);
            bool redirected;
            bool cookieAdded;
            manager.SaveSessionID(HttpContext.Current, newSessionId, out redirected, out cookieAdded);
            return newSessionId;
        }

        private bool MayChangeSessionId {
            get {
                // cannot change session id for cookieless session
                if (IsCookieless) {
                    return false;
                }

                // Hopefully we'd return true here. But there are yet some problems to be
                // solved that prevent us from enabling the change session id code:
                // - it is not working when we've more than one user provider. In this case,
                //   if you have more than one active session for the same cookie, you can't just
                //   change the session id, because that would wipe out all the sessions sharing the
                //   cookie. In order to make that work, we'd have to update the session id for
                //   all sessions, and in a way that works regardless of the session provider being
                //   used (not only for the DB provider but also for the inproc one)
                // - it hasn't been tested with the inproc provider, and I don't have any good
                //   reasons to think that it is working, namely because of the hacks that had to
                //   be made for the DB provider: the SessionIDManager.SaveSessionID method is only
                //   changing the cookie value, but not the HttpContext.Session.SessionId value.
                //   We account for this in the DB provider, but not for the inproc one
                
                // if this ever changes to true:ChangeSessionCookie method needs to be translated to Java
                return false;
            }
        }
        ...
        private bool IsCookieless {
            get {
                ...
                return appInfo == null ? true : appInfo.OsContext.IsCookielessSession;
            }
        }
        ...
        internal Pair<String,String> GetDatabaseConnectionOverride(String connectionName) {
            if (_databaseConnectionOverrides != null) {
                ...
                if (_databaseConnectionOverrides.TryGetValue(connectionName, out value)) {
                    return new Pair<String, String> (Settings.DecryptString(value.First), value.Second);
                }
            }
            return null;
        }
        ...
        internal void SetDatabaseConnectionOverride(String connectionName, string connectionString, string initialSchema) {
            ...
            _databaseConnectionOverrides[connectionName] = new Pair<string, string> (Settings.EncryptString(connectionString), 
                initialSchema);
        }
        ...
        private void Logout(int userId) {
            ...
            using (Transaction trans = DatabaseAccess.ForSystemDatabase.GetRequestTransaction()) {
	            DBRuntimePlatform.Instance.DeletePersistentLogin(trans, userId, TerminalType);
            }
        ...
        private volatile static int defaultTenant = 0;
                    ...
                    lock( _variables) {
                        if (_variables[TENANT_ID] == null) {
                            ...
                            if (currentDefaultTenant == 0) {
                                currentDefaultTenant = RuntimePlatformUtils.GetCurrentDefaultTenantId();
    ...
    public class HeContext { // He prefix to avoid the need of prefixing with namespace (conflicts)
        ...
        public MobileOriginatedMessage MOMsg;
        public MobileTerminatedMessage MTMsg;
        ...
        public SessionInfo Session {
            get {
                ...
                if (_context.Session == null) {
                    if (_tmpSession == null) _tmpSession = new SessionInfo();
                    return _tmpSession;
                }
                ...
                try {
                    ...
                    try {
                        Log.ErrorLog.StaticWrite(DateTime.Now, "", (_appInfo != null ? _appInfo.eSpaceId : 0),
                            0, 0, "Error obtaining session data." + ex.Message, ex.StackTrace, Log.ErrorLog.GetStackEnvironmentInfo(_appInfo, this), "RuntimePlatform");
                    } catch { }
                }
        ...
        public bool IsCookielessSession {
            get {
                return _context.Session == null ? true : _context.Session.CookieMode != HttpCookieMode.UseCookies;
            }
        }
        ...
        private static Dictionary<Pair<string,int>,int> _roleIdCache = null;
        ...
        private void Constructor(HttpContext context) {
            ...
            if (_appInfo != null) {
                _currentESpaceId = _appInfo.eSpaceId;
            }
            ...
            if (SafeGetRequest(context) != null && SafeGetRequest(context).Params != null) {
                if ((context.Request.Params["msisdn"] != null) || (context.Request.Params["la"] != null)) {
                    MOMsg = SmsUtils.BuildMsgFromRequest(context.Request);
                }
            }
        }
        ...
        public string GetExcelTemplateFilePath() {
            return Settings.Get(RuntimePlatform.Settings.Configs.ExcelTemplateFilePath);
        }
        ...
        public int CurrentESpaceId {
            get {
                return _currentESpaceId;
            }
            set {
                _currentESpaceId = value;
            }
        }
        ...
        internal bool SessionWasCleared {
            get { return sessionWasCleared; }
        }

        internal void OnSessionCleared() {
            this.sessionWasCleared = true;
        }

        internal void OnSessionIDChanged(string newSessionId) {
    ...
    /// </summary>
    [Serializable]
    class SessionHashtable : ISerializable {
        // the session
        private string sessionID;
        public string SessionID {
            get {

...
namespace OutSystems.HubEdition.RuntimePlatform.Db {
    ...
    public class ExcelResultSet : IDataReader {
        ...
        private static class CompressingUtils {
            ...
            public static bool Unzip(byte[] zipFile, string destinationPath) {
                if (!HasZipHeader(zipFile)) {
                    return false;
                }

                FileStream outputFile = null;
                try {
                    MemoryStream inputBuffer = new MemoryStream(zipFile);
                    ZipInputStream unzipper = new ZipInputStream(inputBuffer);
                    ZipEntry entry;
                    int bytesRead;
                    while ((entry = unzipper.GetNextEntry()) != null) {
                        byte[] data = new byte[1024];
                        if (!entry.IsDirectory) {
                            String fullName = destinationPath + Path.DirectorySeparatorChar + entry.Name;
                            Directory.CreateDirectory(Path.GetDirectoryName(fullName));
                            outputFile = File.Create(fullName);
                            while ((bytesRead = unzipper.Read(data, 0, data.Length)) > 0) {

...
namespace OutSystems.HubEdition.RuntimePlatform.Email {
    ...
    public class EmailHelper {
        ...
        internal static string CreateRandomBoundary() {
            return "----=_NextPart_:_" + System.Guid.NewGuid().ToString();
        } // GetRandomBoundary
        ...
        public static void HttpGet(string ssUrl, string userAgent, Cookie cookie, out string ssContent, out string ssContentEncoding) {
            HttpGetContent(ssUrl, null, null, userAgent, cookie, null, out ssContent, out ssContentEncoding);
        }

        public static void HttpPost(string ssUrl, QueryParameter[] parameters, string userAgent, Cookie cookie, out string ssContent, out string ssContentEncoding) {
            HttpGetContent(ssUrl, "POST", "application/x-www-form-urlencoded", userAgent, cookie, parameters, out ssContent, out ssContentEncoding);
        }
        ...
        public static void HttpBinaryGet(string ssUrl, string userAgent, out string ssContentType, out byte[] ssContent) {
            ...
            int chunkLen = 10240; // lets try to read chunks of 10k
            ...
            using (HttpWebResponse resp = (HttpWebResponse)req.GetResponse()) {
                ...
                ArrayList contentArray = new ArrayList(resp.ContentLength > 0 ? (int)resp.ContentLength : chunkLen);

                if (RuntimePlatformUtils.IsJava()) {
                    ssContent = GetJavaBinary(resp.GetResponseStream());
                } else {
                    using (BinaryReader sr = new BinaryReader(resp.GetResponseStream())) {
                        // we read in chunks instead of using ContentLength because that header
                        // isn't set by every server (eg: <?php echo('hello'); flush(); echo('world'); ?>)
                        ssContent = new byte[resp.ContentLength];
                        byte[] chunk = null;
                        do {
                            chunk = sr.ReadBytes(chunkLen);
                            contentArray.AddRange(chunk);
                ...
                ssContent = (byte[])contentArray.ToArray(typeof(byte));
                ssContentType = resp.ContentType;
            }
        } // HttpBinaryGet

        // Java method for reading byte[] from web response
        private static byte[] GetJavaBinary(Stream inputStream) {
            return null;
        }
                ...
                while (en.MoveNext()) {
                    ...
                    if (absUri != null) {
                        ...
                        try {
                            ...
                            EmailHelper.HttpBinaryGet(absoluteUrl, userAgent, out contentType, out content);
                        } catch (Exception e) {
        ...
        private static Uri GetAbsolutePathFromRequest(Uri request, string filename) {
        ...
        private static Uri GetAbsolutePathFromRequest(Uri request, string filename, string hostName) {
            ...
            if (GetParentFolder(filename) != "" && GetParentFolder(filename) != "img") {
                ...
                url = GetApplicationPathFromUri(request).ToString();
                url = hostName == null ? url : url.Replace("/127.0.0.1/", "/" + hostName + "/");
                return new Uri(GetApplicationPathFromUri(request) + (filename.StartsWith("/") ? "" : "/") + filename);
            }
        }

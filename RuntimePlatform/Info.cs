...
namespace OutSystems.HubEdition.RuntimePlatform {
    ...
    public class SitePropertiesInfoReadOnly {

        protected class SiteProperty {
            ...
            public SiteProperty(object val) : this(0, null, true, val) {
            }

            public SiteProperty(int defId, ObjectKey key, bool isShared, object val) {
                DefId = defId;
                Key = key;
                IsShared = isShared;
                Value = val;
        ...
        protected Dictionary<string, SiteProperty> _properties;
		...
		public object getSafe( string name ) {
            ...
            SiteProperty prop = _properties.GetValueOrDefault(name);
            if (prop == null) {
                return null;
			}
        ...
        [Obsolete("Deprecated. Site Property Names no longer prefixed by eSpaceName and should be accessed by Global.SiteProperties[name] instead.")]
        public static string BuildSitePropertyName(string eSpaceName, string name) {
            if (name.ToLowerInvariant() != "tenantname" && name.ToLowerInvariant() != "tenantid") {
                return eSpaceName + "." + name;
            } else {
                return name;
            }
        }
        ...
        protected void loadProperties() {
            if (_properties == null) {
                _properties = new Dictionary<string, SiteProperty>(StringComparer.OrdinalIgnoreCase);
                // Add the system properties
                _properties.Add("TenantName", new SiteProperty(TenantName));
                _properties.Add("TenantId", new SiteProperty(TenantId));
            }
            ...
            using (Transaction tran = DatabaseAccess.ForSystemDatabase.GetReadOnlyTransaction()) {
                using (IDataReader reader = DBRuntimePlatform.Instance.GetSitePropertiesInfo(tran, eSpaceId, TenantId)) {
                    while (reader.Read()) {
                        ...
                        object propValue = RuntimePlatformUtils.ConvertFromString(reader.SafeGet<string>("Value", ""), Convert.ToString(reader["Data_Type"]));
                        string name = Convert.ToString(reader["Name"]);
                        _properties[name] = new SiteProperty(defId, propKey, isShared, propValue);
                }
                }
            }
        }
        ...
        protected void SitePropRemovedCallback(string key, Object val, CacheItemRemovedReason reason) {
            lock (this) {
                Valid = false;
    ...
    public class SitePropertiesInfo : SitePropertiesInfoReadOnly {
        ...
        public override object this[string name] {
            set {
                if (_properties.GetValueOrDefault(name) != null) {
                    HeContext heContext = AppInfo.GetAppInfo().OsContext;
                    SiteProperty prop = (SiteProperty)_properties[name];
                    // Check if it is a system var
                    if (prop.Key == null) {
                        throw (new ArgumentException("The site property '" + name + "' is a system property and cannot be changed."));
                    }
                    GenericExtendedActions.UpdateSiteProperty(heContext, prop.DefId, prop.IsShared, value);
                    if (prop.Value != value) {
                        prop.Value = value;
        ...
        public void ClearSitePropertiesInfo() {
    ...
    public class ExtensionPropertiesCache {
        ...
        private void initCache() {
            HttpRuntime.Cache.Insert(
                ...
                TimeSpan.Zero,
                CacheItemPriority.NotRemovable,
                new CacheItemRemovedCallback(extensionPropertiesRemovedCallback));
        }
    ...
    public class EspaceProperties {
        ...
        public bool EnableEmails {
            get {
                return getPropWithCachedDefault<bool>(Settings.Configs.Email_EnableEmails,
                                                      value => value.ToLower() == "true",
                                                      () => Settings.Get(Settings.Configs.Email_EnableEmails));
            }
        }

        public bool EspaceApplicationEnabled {
            get {
                return getPropWithCachedDefault<bool>("ApplicationEnabled", value => value.ToLower() == "true",
                    () => {
                        return RuntimePlatformUtils.IsApplicationEnabled(this.m_parentInfo.eSpaceId).ToString();
                    });
            }
        }
        ...
        private void loadProperties() {
            ...
            using (Transaction trans = DatabaseAccess.ForSystemDatabase.GetReadOnlyTransaction()) {
                using (IDataReader reader = DBRuntimePlatform.Instance.GetEspaceConfigInfo(trans, m_parentInfo.eSpaceId)) {
        ...
        public void initCache() {
            HttpRuntime.Cache.Insert(
                "eSpacePropertiesRemCondition" + m_parentInfo.eSpaceId,
                m_parentInfo.eSpaceId,
                AppInfo.CalculateCacheDependency(m_parentInfo.eSpaceId, 0),
                DateTime.Now.AddDays(1),
                TimeSpan.Zero,
                CacheItemPriority.NotRemovable,
                new CacheItemRemovedCallback(eSpacePropertiesRemovedCallback));
        }
    ...
    public class TenantInfo {
        ...
        // Only contains data for the entry espace as it is not possible to send or receive sms inside a producer action
        private IDictionary<ObjectKey, string> _phoneNumbers = new Dictionary<ObjectKey, string>();

        // Only contains data for the entry espace as it is not possible to send or receive sms inside a producer action
        private IDictionary<string, IList<ObjectKey>> _phoneNodes = new Dictionary<string, IList<ObjectKey>>();


        //private IDictionary<int, IDictionary<ObjectKey, string>> _producerPhoneNumbers = new Dictionary<int, IDictionary<ObjectKey, string>>();


        //private IDictionary<int, IDictionary<string, IList<ObjectKey>>> _producerPhoneNodes = new Dictionary<int, IDictionary<string, IList<ObjectKey>>>();

        private AppInfo _parentApp;

        private static Hashtable locker = Hashtable.Synchronized(new Hashtable());
        ...
        private void initCache() {
            lock (GetLockForTenant(this.Id)) {
                ...
                HttpRuntime.Cache.Insert("tenantInfoRemCondition" + _tenantId, _tenantId, dependencies, DateTime.Now.AddDays(1), TimeSpan.Zero, CacheItemPriority.NotRemovable, new CacheItemRemovedCallback(TenantInfoRemovedCallback));
                inInitCache = false;
            }
        }
        ...
        private void TenantInfoRemovedCallback(string key, Object val, CacheItemRemovedReason reason) {
            lock (GetLockForTenant(this.Id)) {
                ...
                _phoneNumbers = new Dictionary<ObjectKey, string>();
                _phoneNodes = new Dictionary<string, IList<ObjectKey>>();
                //_producerPhoneNumbers = new Dictionary<int, IDictionary<ObjectKey, string>>();
                //_producerPhoneNodes = new Dictionary<int, IDictionary<string, IList<ObjectKey>>>();
        ...
        private void LoadPhoneNumbers() {
            IDictionary<ObjectKey, string> tenantPhoneNumbers;
            IDictionary<string, IList<ObjectKey>> tenantPhoneNodes;
            _phoneConfigException = null;
            ...
            using (Transaction tran = DatabaseAccess.ForSystemDatabase.GetReadOnlyTransaction()) {
                ...
                using (IDataReader reader = DBRuntimePlatform.Instance.GetSMSEntryNodes(tran, this.Id, this._parentApp.eSpaceId)) {
                    while (reader.Read()) {
                        ...
                        if (tenantPhoneNumbers.ContainsKey(node)) {
                            tenantPhoneNumbers.Remove(node);
                        }
                        tenantPhoneNumbers[node] = phone;
                        // add to phone->node list
                        if (tenantPhoneNodes.ContainsKey(phone)) {
                            tenantPhoneNodes[phone].Add(node);
                        } else {
                            List<ObjectKey> al = new List<ObjectKey>();
                            al.Add(node);
                            tenantPhoneNodes[phone] = al;
                        }
                    }
                }
            }
        ...
        public string GetPhoneNumber(ObjectKey numberKey, int espaceId) {
            if (espaceId != _parentApp.eSpaceId) {
                throw new Exception("Can't use producer SMS actions from consumer.");
            }
            checkPhoneNumbersInfoIsValid();
            if (!_phoneNumbers.ContainsKey(numberKey)) {
                LoadPhoneNumbers();
            }
            return _phoneNumbers[numberKey];
        }
    ...
    public sealed class AppInfo {
        ...
        private InjectionCache _injectionCache = null;
        ...
        private Dictionary<int,EspaceInfo> _espaces = new Dictionary<int,EspaceInfo>();
        ...
        public string PtaName {
            get {
                return GetRequestPtaName(HttpContext.Current.Request);
            }
        }
        ...
        public string RequestId {
            get {
                return (string)Context.Items["osRequest"];
            }
        }
        ...
        [ThreadStatic] private static bool insertingCache = false;
        ...
        public void SetupInvalidateCallbackCache() {
            if (!insertingCache && HttpRuntime.Cache.Get("callbackCache" + eSpaceId) == null) { // prevent double inserts and overflows
                ...
                HttpRuntime.Cache.Insert(
                    ...
                    TimeSpan.Zero,
                    CacheItemPriority.NotRemovable,
                    new CacheItemRemovedCallback(InvalidateCallbackCacheCallback));
        ...
        public static string GetRequestPtaName(HttpRequest request) {
            // Note: The first element of the path is empty
			string[] path = request.ApplicationPath.Split('/');
			Trace.Assert(path.Length <= 3, "Invalid path: " + request.ApplicationPath);

...
namespace OutSystems.HubEdition.RuntimePlatform {
    ...
    public partial class RuntimePlatformUtils {
        ...
        private static bool ExtensionTresholdKnown = false;
        ...
        private static void LoadInternalIPAddresses() {
            lock (typeof(RuntimePlatformUtils)) {
                string ipList = Settings.Get(RuntimePlatform.Settings.Configs.InternalNetwork_IPAdresses);
                ...
                foreach (string ip in ipList.Split(';')) {
                    if (ip != "") {
                        if (IPAddressValidation(ip.Trim())) {
        ...
        public static string GetRequestRealIP(HttpRequest request) {
            // Step over proxy server
            string requestIp = request.Headers.Get("X-Forwarded-For");
        ...
        private static string GetWebReferenceEffectiveOrDefaultURL(string webrefSSKey, int eSpaceId, bool cacheValue, out bool isDefault) {
            ...
            if (cacheValue) {
                isDefault = false;
                return AppCache.GetESpaceCachedValue<string, string>(webrefSSKey, "WebReferenceURLCache", eSpaceId, _ => fetchFromDb());
            } else {

...
namespace OutSystems.HubEdition.RuntimePlatform.Db {
    ...
    public abstract class OSList<T> : IOSList {
        ...
        /// </remarks>
#if JAVA
        protected IList<T> _data = new ArrayList<T>();
#else
        ...
        public OSList(IDbTransaction trans, ReadDBMethodDelegate alternateReadDBMethod) : this() {
            if (trans != null) {
                try {
                    ...
                    Transaction = new ManagedTransaction(DatabaseAccess.GetProviderForTransaction(trans).TransactionManager, trans);
                } catch (Exception e) {
                    /* 
    ...
    /// </summary>
    [Serializable()]
    public class GenericRecordList<T> : OSList<T>, RecordList where T : ITypedRecord<T> {
        ...
        protected override OSList<T> NewList() {
            return new GenericRecordList<T>();
        }
    ...
    public sealed class BasicTypeList<T> : OSList<T> {
        ...
        public static OtherType[] ToArray<OtherType>(BasicTypeList<T> rl, Func<T, OtherType> converter) {
            ...
            OtherType[] array = new OtherType[rl.Length];
            int i = 0;
            foreach (T item in rl.Data) {

        ...
        private AbstractCallbackInvoke CreateCallbackInvoke(IDataReader reader) {
            ...
            if (toLowerUrl.TrimStart().StartsWith("http://") || toLowerUrl.TrimStart().StartsWith("https://")) {
                if (RuntimePlatformUtils.IsJava()) {
				 ...
				 /* 
					// removing/commenting code of #119946
                     else if (toLowerUrl.StartsWith("http://localhost/") || toLowerUrl.StartsWith("https://localhost/") ||
                           toLowerUrl.StartsWith("http://127.0.0.1/") || toLowerUrl.StartsWith("https://127.0.0.1/")) {

                    //this "localhost" should be improved to support sandbox scenarios for ECT and BPM
                    return new ForeignCallbackProviderInvoke(url, serviceName, providerName, methodName);
                	} 
				*/ 
				else {
                    return new WebServiceCallbackInvoke(url, serviceName, providerName, methodName);

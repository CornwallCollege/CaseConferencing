    ...
    public class GenericExtendedActions {
        ...
        public static void Grant(HeContext heContext, ObjectKey key, bool isPersistent, int userId, int eSpaceId) {
                ...
                if( userId == heContext.Session.UserId) {
                    ...
                    heContext.Session.AddRole(heContext.GetRoleIdFromKey(key.ToString(ObjectKey.DatabaseFormat), eSpaceId));
                }
        ...
        public static void UpdateSiteProperty(HeContext heContext, int definitionId, bool isShared, object val) {
            string propValue;
            int tenantId = heContext.AppInfo.Tenant.Id;

            using (Transaction tran = DatabaseAccess.ForSystemDatabase.GetRequestTransaction()) {
                propValue = RuntimePlatformUtils.ConvertToString(val);
                DBRuntimePlatform.Instance.UpdateSiteProperty(tran, definitionId, isShared, tenantId, propValue);
            }
        }
        ...
        //private static ICompilerContext GetCompilerContext() {
        //    ICompilerContext context = new ICompilerContext();
        //    context.SandboxKey = Settings.Get(Settings.Configs.Sandbox_BoxKeyName);
        //    return context;
        //}
        ...
        private static void AttemptLogin(HeContext heContext, int inParamUserId, string inParamPassword, bool inParamPersistent, bool checkPassword) {
            ...
            using (Transaction trans = DatabaseAccess.ForSystemDatabase.GetRequestTransaction()) {
                sql = "SELECT USERNAME, PASSWORD FROM OSSYS_User " +
                      "WHERE Tenant_Id = @Tenant_Id AND Is_Active = 1 AND " +
                                DatabaseAccess.ForSystemDatabase.DatabaseServices.DMLService.Identifiers.EscapeIdentifier("ID") + " = @Id";
                sqlCmd = trans.CreateCommand(sql);
                sqlCmd.CreateParameter("@Tenant_Id", DbType.Int32, tenantId);
                sqlCmd.CreateParameter("@Id", DbType.Int32, inParamUserId);
                ...
                if (checkPassword && !RuntimePlatformUtils.Hashing.ValidatePasswordAgainstHash(inParamPassword, hashInDb)) {
                    throw new InvalidLoginException("Invalid login");
                }
            ...
            using (Transaction trans = DatabaseAccess.ForSystemDatabase.GetRequestTransaction()) {
                sqlCmd = trans.CreateCommand();

                // if it is a persistent login create new entry 
                if (inParamPersistent) {
                    int daysToRememberLogin = Settings.GetInt(Settings.Configs.RememberLoginTimeoutInDays);

                    string expiresExpression = DatabaseAccess.ForSystemDatabase.DatabaseServices.DMLService.Functions.AddDays("GETDATE()", 
                        daysToRememberLogin.ToString(CultureInfo.InvariantCulture));

                    sql = "INSERT INTO OSSYS_Persistent_Login " +
                          "( Tenant_Id, Secret, Expires, User_Id, Terminal_Type) VALUES ( @Tenant_Id, @Secret, " + expiresExpression + ", @User_Id, @Terminal_Type)";
                    ...
                    switch (heContext.Session.TerminalType) {
                        ...
                        case "WEB":
                            ...
                            if (!String.IsNullOrEmpty(info.UserProviderName)) {
                                cookieName = info.UserProviderName;
                            } else {
                                cookieName = info.eSpaceName;
                                if (info.IsMultiTenant) {
                                    cookieName += "/" + info.Tenant.Name;
                                }
                            }

                            var cookie = new HttpCookie(cookieName, DateTime.Now.ToString(CultureInfo.CurrentCulture) + inParamUserId + random.Next());
                            var timeSpan = new TimeSpan(daysToRememberLogin, 0, 0, 0);

                            
                            cookie.Expires = DateTime.Now.Add(timeSpan);

                            sqlCmd.CreateParameter("@Secret", DbType.String, cookie.Value);
                    ...
                    sqlCmd.CreateParameter("@User_Id", inParamUserId);
                    sqlCmd.CreateParameter("@Terminal_Type", DbType.String, heContext.Session.TerminalType);
                    sqlCmd.CommandText = sql;

                    sqlCmd.ExecuteNonQuery(String.Format("{0} operation (insert persistency info)", functionName), true);
                }

                // update login date

                bool passwordUpgradeOnLogin = checkPassword && (inParamPassword != null) && Settings.GetBool(Settings.Configs.PasswordUpgradeOnLogin)
                    && RuntimePlatformUtils.Hashing.GetAlgorithmIndexForPasswordHash(hashInDb) < RuntimePlatformUtils.Hashing.BestAlgorithmIndex;

                // #798465 Use a commitable transaction to update the last login to avoid locking concurrent logins
                using (Transaction committableTrans = DatabaseAccess.ForSystemDatabase.GetCommitableTransaction()) {
                    try {
                        ...
                        var commitSql = "UPDATE OSSYS_User SET Last_Login = @Last_Login " +
                                        (passwordUpgradeOnLogin ? ", Password = @Password " : "") +
                        ...
                        if (passwordUpgradeOnLogin) {
                            commitCmd.CreateParameter("@Password", DbType.String, RuntimePlatformUtils.Hashing.GeneratePasswordHashWithBestAlgorithm(inParamPassword));
                        }
                        commitCmd.CreateParameter("@Tenant_Id", DbType.Int32, tenantId);
                        commitCmd.CreateParameter("@User_Id", DbType.Int32, inParamUserId);
                        commitCmd.CommandText = commitSql;
                        commitCmd.CommandTimeout = 5; /*5 seconds*/
                    
                        commitCmd.ExecuteNonQuery("%FUNCTION_NAME% operation (update last login date)", true);
